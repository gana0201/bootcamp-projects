...
export default function FitReportView({
  ...
  const handleTryOn = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || "가상 피팅 실패");
      }
      ...
      if (data.mock) {
        setTryOnError(
          "Mock 모드: 현재 전신 사진이 표시됩니다. 유료 API 연동 시 실제 합성 이미지가 생성됩니다."
        );
      }
    } catch (err) {
      setTryOnError(
  ...
  return (
    <div className="w-full space-y-6">
      {/* Two Column Layout: Left sticky, Right flows naturally */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        
        {/* LEFT: Virtual Try-On (sticky on desktop) + Body Analysis below */}
        <div>
                ...
                {tryOnImage ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    {tryOnError && !tryOnLoading && (
                      <p className="mt-3 text-sm text-amber-600">{tryOnError}</p>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <InfoItem
                  label="어깨"
                  value={
                    bodyAnalysis.shoulderWidth === "wide"
                      ? "넓음"
                      : bodyAnalysis.shoulderWidth === "narrow"
                      ? "좁음"
                      : "보통"
                  }
                />
        ...
        {/* RIGHT: Report Content (scrollable frame) */}
        <div className="space-y-5 lg:h-[calc(100vh-180px)] lg:overflow-y-auto lg:pr-2">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-md p-6">
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-4 flex flex-col items-center justify-center text-center min-w-[120px]">
                ...
                <span className={`text-[11px] font-bold mt-2 px-2.5 py-0.5 rounded-full border ${
                  report.fitScore >= 7 ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
                  report.fitScore >= 5 ? "bg-amber-50 text-amber-700 border-amber-200" :
              ...
              <div className="flex-1 space-y-2.5">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-lg font-black text-blue-600 bg-blue-50 border border-blue-200 px-3 py-0.5 rounded-lg">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-bold text-slate-800">
                    {garmentData.name} · {garmentData.category === "top" ? "상의" : garmentData.category === "bottom" ? "하의" : garmentData.category === "outer" ? "아우터" : "원피스"}
                  </span>
                </div>
                {/* Progress Bar */}
                <div className="pt-1">
                  <div className="h-2.5 w-full rounded-full bg-slate-200 overflow-hidden">
                    <div
                      ...
                      style={{ width: `${report.fitScore * 10}%` }}
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="divide-y divide-slate-100">
              {Object.entries(report.details).map(([key, value]) => (
                <div key={key} className="py-3.5 flex items-start gap-3">
                  <span className="text-xs font-extrabold text-blue-700 bg-blue-50 border border-blue-200 px-2.5 py-1 rounded-md uppercase shrink-0 mt-0.5">
                    {key}
                  </span>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
              ...
              <div className="space-y-3">
                {report.styling.map((tip, i) => (
                  <div
                    key={i}
                    className="flex gap-3 text-base text-slate-700 font-medium bg-emerald-50 p-4 rounded-xl border border-emerald-200"
                  >
                    <div className="p-1 bg-emerald-100 text-emerald-600 rounded-lg shrink-0 h-fit">
          ...
          {report.cautions.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
              ...
              <div className="space-y-3">
                {report.cautions.map((caution, i) => (
                  <div
                    key={i}
                    className="flex gap-3 text-base text-slate-700 font-medium bg-amber-50 p-4 rounded-xl border border-amber-200"
                  >
                    <div className="p-1 bg-amber-100 text-amber-600 rounded-lg shrink-0 h-fit">
          ...
          <div className="flex gap-3 p-4 bg-slate-50 border border-slate-200 text-slate-500 rounded-xl text-sm leading-relaxed">
            ...
            <span>
              본 리포트는 AI가 입력된 신체 정보와 의류 실측 패턴을 비교 대조하여 생성한 분석 결과입니다. 실제 착용 핏과 소재 특성에 따라 오차가 발생할 수 있습니다.
            </span>
          </div>
        </div>
      </div>
