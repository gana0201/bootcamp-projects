...
export default function GarmentForm({ onSubmit, onBack }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
      {/* Left Card: Clothing Photo Upload + Category */}
      <div className="bg-white/95 rounded-2xl border border-brand-cream/60 shadow-sm p-6 flex flex-col justify-between h-full space-y-6">
        <div className="space-y-6 flex-1 flex flex-col">
          ...
          <div className="flex-1 flex flex-col justify-center">
            <div
              ...
              onDrop={handleDrop}
              onClick={() => {
                const input = document.getElementById("garment-photo-input") as HTMLInputElement;
                input?.click();
              }}
            ...
            >
              <input
                ...
                onChange={(e) => {
                  ...
                  if (file) handleGarmentImage(file);
                }}
                disabled={uploading}
              />
              ...
              {garmentImage ? (
                <div className="flex flex-col items-center justify-center w-full space-y-3">
                  <div className="relative rounded-xl overflow-hidden bg-white border border-brand-cream shadow-sm p-3 max-w-[180px] group">
                    ...
                    <div className="absolute inset-0 bg-brand-dark/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-xl">
                      <span className="text-sm font-bold text-white bg-brand-dark/90 px-2.5 py-1.5 rounded-full border border-white/20">
                        클릭하여 사진 변경
                      </span>
                    </div>
                  </div>
                  <div className="text-center">
                    {uploading ? (
                      ...
                      <span className="text-sm font-bold text-brand-primary flex items-center justify-center gap-1 bg-brand-light border border-brand-cream/60 px-3 py-1 rounded-full w-fit mx-auto">
                        <Check className="w-4 h-4 text-brand-secondary" /> 의류 사진 업로드 완료
                      </span>
                    ) : null}
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center gap-3 py-4">
                  <div className="w-14 h-14 rounded-full bg-brand-secondary/20 flex items-center justify-center text-brand-secondary shadow-inner">
        ...
        <div className="flex gap-2 p-3 bg-brand-secondary/15 text-brand-dark rounded-xl text-sm leading-relaxed border border-brand-secondary/35">
          ...
          <span>
            <strong>의류 사진 팁:</strong> 바닥에 반듯하게 펼쳐진 옷 사진이 가장 정밀한 분석 결과를 제공합니다.
          </span>
        </div>
      </div>

      {/* Right Card: Measurements */}
      <div className="bg-white/95 rounded-2xl border border-brand-cream/60 shadow-sm p-6 flex flex-col justify-between h-full space-y-6">
        <div className="space-y-6 flex-1 flex flex-col justify-between">
          <div className="space-y-5">
            <div className="flex items-center justify-between pb-3 border-b border-brand-cream/35">
              ...
              <span className="text-xs font-extrabold text-brand-primary bg-brand-light border border-brand-cream/60 px-2.5 py-1.5 rounded-full">
            ...
            <div className="space-y-3.5">
              {currentMeasurements.map((measurement) => (
                <div key={measurement} className="space-y-1.5">
                  <label className="text-sm font-bold text-brand-dark/70 block">{measurement}</label>
          ...
          <div className="bg-brand-light border border-brand-cream/50 p-4 rounded-xl text-sm leading-relaxed text-brand-dark/85 flex gap-2">
            ...
            <div>
              ...
              평평한 곳에 의류를 펼쳐두고 단면 치수를 cm 단위로 기입하세요. 사진만 올려도 AI가 추정 분석합니다.
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          <button
            type="button"
            onClick={onBack}
            className="flex-1 py-3.5 border border-brand-cream text-brand-dark font-extrabold text-sm rounded-xl hover:bg-brand-light/40 active:scale-95 transition-all"
          >
            &larr; 1단계로 이동
          </button>
          <button
