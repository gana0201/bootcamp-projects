...
export default function GarmentForm({ onSubmit, onBack }: Props) {
  ...
  const handleGarmentImage = useCallback(async (file: File) => {
    ...
    const reader = new FileReader(); reader.onload = (e) => setGarmentImage(e.target?.result as string); reader.readAsDataURL(file);
    setUploading(true);
    try { const formData = new FormData(); formData.append("file", file); const res = await fetch("/api/upload", { method: "POST", body: formData }); if (!res.ok) throw new Error("업로드 실패"); const data = await res.json(); setGarmentImageUrl(data.imageUrl); }
    catch { alert("의류 사진 업로드 실패"); setGarmentImage(""); } finally { setUploading(false); }
  }, []);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const numericMeasurements: Record<string, number> = {};
    for (const key of currentMeasurements) { const val = parseFloat(measurements[key] || ""); if (!isNaN(val) && val > 0) numericMeasurements[key] = val; }
    ...
    <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
            ...
            <div className="flex gap-2 flex-wrap">
              {Object.entries(CATEGORY_LABELS).map(([key, label]) => (
                <button key={key} type="button" onClick={() => handleCategoryChange(key)}
                  className={`py-2 px-4 rounded-full text-sm font-bold transition-all border ${category === key ? "bg-brand-primary border-brand-primary text-white shadow-md" : "bg-brand-light border-brand-cream text-slate-700 hover:border-brand-primary"}`}>{label}</button>
              ))}
            </div>
          </div>
          <div className="flex-1 flex flex-col justify-center">
            <div onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const file = e.dataTransfer.files?.[0]; if (file?.type.startsWith("image/")) handleGarmentImage(file); }}
              onClick={() => { const input = document.getElementById("garment-photo-input") as HTMLInputElement; input?.click(); }}
              className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all flex flex-col items-center justify-center flex-1 min-h-[180px] ${garmentImage ? "border-brand-primary bg-brand-primary/5" : "border-brand-cream hover:border-brand-primary hover:bg-brand-light"}`}>
              <input type="file" id="garment-photo-input" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) handleGarmentImage(file); }} disabled={uploading} />
              {garmentImage ? (
                <div className="flex flex-col items-center w-full space-y-3">
                  ...
                  : garmentImageUrl ? <span className="text-sm font-bold text-brand-primary flex items-center gap-1"><Check className="w-4 h-4 text-brand-secondary" /> 업로드 완료</span> : null}
                </div>
              ) : (
      ...
      <div className="bg-white rounded-2xl border border-brand-cream shadow-sm p-6 flex flex-col justify-between h-full space-y-6">
        <div className="space-y-6 flex-1 flex flex-col justify-between">
          <div className="space-y-5">
            <div className="flex items-center justify-between pb-3 border-b border-brand-cream">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-base font-bold text-slate-800">2-2. 의류 실측 스펙 입력</h2>
                  <p className="text-sm text-slate-500">아는 부분만 입력해도 분석 가능</p>
                </div>
              </div>
              ...
              {currentMeasurements.map((m) => (
                <div key={m} className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input type="number" step="0.5" min="0" value={measurements[m] || ""} onChange={(e) => setMeasurements((prev) => ({ ...prev, [m]: e.target.value }))}
                      className="w-full bg-brand-light border border-brand-cream focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/10 rounded-xl px-4 py-3 text-base font-bold text-slate-800 outline-none pr-12" placeholder="-" />
          ...
          <div className="bg-brand-primary/5 border border-brand-primary/15 p-4 rounded-xl text-sm text-slate-600 flex gap-2">
            <Info className="w-4 h-4 text-brand-primary shrink-0 mt-0.5" />
            <span><strong>권장:</strong> 평평한 곳에 의류를 펼쳐두고 cm 단위로 측정. 사진만 올려도 AI가 추정합니다.</span>
          </div>
        </div>
        <div className="flex gap-3 pt-2">
          <button type="button" onClick={onBack} className="flex-1 py-3.5 border border-brand-cream text-slate-600 font-extrabold text-sm rounded-xl hover:bg-brand-light active:scale-95 transition-all">&larr; 1단계로</button>
          <button type="submit" disabled={uploading}
            className="flex-1 px-6 py-3.5 bg-brand-accent hover:bg-brand-accent/90 text-white font-extrabold text-base rounded-xl shadow-lg shadow-brand-accent/20 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed">분석 시작 <ArrowRight className="w-5 h-5" /></button>
        </div>
      </div>
    </form>
  );
